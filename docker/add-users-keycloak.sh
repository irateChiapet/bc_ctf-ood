...
LOCK_FILE="/tmp/keycloak_users_created.lock"

echo "Starting Keycloak user creation using HTTPS on port ${KEYCLOAK_PORT}..."

# Check if script has already been run successfully
if [ -f "$LOCK_FILE" ]; then
    echo "Users have already been processed (lock file exists: $LOCK_FILE)"
    echo "To force re-run, delete the lock file: rm $LOCK_FILE"
    exit 0
fi
...
/opt/keycloak/bin/kcadm.sh config credentials \
    --server "https://${HOSTNAME}:${KEYCLOAK_PORT}" \
    --realm master \
    --user "${KEYCLOAK_ADMIN}" \
...
if [ -f /opt/keycloak/templates/passwd ]; then
    ...
    while IFS=: read -r username password uid gid gecos homedir shell; do
        ...
        existing_user_response=$(/opt/keycloak/bin/kcadm.sh get users -r "${REALM_NAME}" --query username="${username}" --fields id,username --insecure 2>/dev/null || echo "")
        existing_user_id=$(echo "$existing_user_response" | grep '"id"' | cut -d'"' -f4 | head -1)
        ...
        if [ -n "$existing_user_id" ] && [ "$existing_user_id" != "null" ]; then
            ...
            if /opt/keycloak/bin/kcadm.sh update users/${existing_user_id}/reset-password -r "${REALM_NAME}" \
                -s type=password -s value="${user_password}" -s temporary=false -n --insecure 2>/dev/null; then
            ...
            /opt/keycloak/bin/kcadm.sh update users/${existing_user_id} -r "${REALM_NAME}" \
                ...
                -s lastName="${lastname}" \
                -s "attributes.jobTitle=[\"${jobtitle}\"]" --insecure 2>/dev/null || true
            ...
            echo "  User does not exist, creating new user: $username"
...
EOF
            ...
            if /opt/keycloak/bin/kcadm.sh create users -r "${REALM_NAME}" -f "$temp_user_file" --insecure 2>/dev/null; then
...
echo "$(date): Keycloak users processed successfully" > "$LOCK_FILE"
echo "Created lock file: $LOCK_FILE"

# List users in realm for verification
echo ""
echo "Verifying users in realm ${REALM_NAME}:"
total_users=$(/opt/keycloak/bin/kcadm.sh get users -r "${REALM_NAME}" --fields username --insecure 2>/dev/null | grep '"username"' | wc -l || echo 0)
